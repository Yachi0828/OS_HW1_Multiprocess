...
void writeFile(string outputnum, vector <int> outVec){
    ...
    outFile << "Output Time: " <<buff<< "." <<ts.tv_nsec <<"+08:00"<<endl;
    outFile.close();
}
...
int main(int argc, const char * argv[]) {
    ...
    cout << "請輸入要切成幾份: "<<endl;
    cin >> divideNum;
    cout << "請輸入方法編號：(方法一, 方法二, 方法三, 方法四)"<<endl;
    ...
    if(command == 1 ){
        ...
        threads1.reserve(divideNum);
        for (int i = 0; i < divideNum; i++) {
            threads1.emplace_back(bubbleSort, ref(dataToKpart.at(i)));
        }
